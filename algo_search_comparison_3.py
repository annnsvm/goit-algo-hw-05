...
def run_3_tests(text, pattern):
    results = []
    time = benchmark(boyer_moore_search, text, pattern)
    results.append(('Боєра-Мура', pattern, time))
    time = benchmark(knuth_morris_pratt_search, text, pattern)
    results.append(('Кнута-Морріса-Пратта', pattern, time))
    time = benchmark(rabin_karp_search, text, pattern)
    results.append(('Рабіна-Карпа', pattern, time))
    return results

def draw_table(results):    
    print(f"\n\n| {'Алгоритм':<21} | {'Підрядок':<28} | {'Час виконання'}")    
    print(f"| {'-'*21} | {'-'*28} | {'-'*13}")
    for result in results:
        print(f"| {result[0]:<21} | {result[1]:<28} | {result[2]:<13.6f}")

if __name__ == '__main__':
    article1 = read_file('article_1.txt')
    res = run_3_tests(article1, "Жадібний алгоритм")
    draw_table(res)
    res = run_3_tests(article1, "неіснуючий текст")
    draw_table(res)
    article2 = read_file('article_2.txt')
    res = run_3_tests(article1, "Профілювання тестового коду")
    draw_table(res)
    res = run_3_tests(article1, "неіснуючий текст")
    draw_table(res)